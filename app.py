    ...
    with right:
        if uploaded:
            if not os.path.exists(model_path):
                st.markdown(f'<div class="rbox r-red">⚠ Model <code>{model_path}</code> not found. Train first.</div>', unsafe_allow_html=True)
            else:
                from tensorflow.keras.preprocessing.image import load_img, img_to_array
                ...
                idx   = int(np.argmax(preds)); conf = float(np.max(preds))
                ...
                if label == "notumor":
                    ...
                    st.markdown('<div class="rbox r-cyan">💡 You\'re clear! Maintain healthy habits and regular check-ups.</div>', unsafe_allow_html=True)
                else:
                    cls = "r-amber" if conf < 0.6 else "r-red"
                    tag = "⚠️ Low Confidence" if conf < 0.6 else "🔴 High Confidence"
                    st.markdown(f'<div class="rbox {cls}">{tag} &nbsp;·&nbsp; Tumor: <strong>{label.upper()}</strong> &nbsp;·&nbsp; {conf*100:.1f}%</div>', unsafe_allow_html=True)
                    st.markdown('<div class="rbox r-amber">⚕️ Consult a neurologist immediately for proper diagnosis.</div>', unsafe_allow_html=True)
                    st.markdown('<div class="rbox r-cyan">💡 Healthy diet &nbsp;·&nbsp; Reduce stress &nbsp;·&nbsp; Follow doctor\'s advice &nbsp;·&nbsp; No self-medication</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="empty-state"><span style="font-size:2.8rem;opacity:.25">🧠</span><p style="color:#6a8a9a;font-size:.9rem;margin:0">Upload an MRI scan to begin analysis</p></div>', unsafe_allow_html=True)
...
with t2:
    ...
    if st.button("🚀  Start Training"):
        ...
        if not train_paths:
            st.markdown(f'<div class="rbox r-red">No data found in <code>{TRAIN_DIR}</code>. Check folder structure.</div>', unsafe_allow_html=True)
        else:
            class_names = sorted(set(train_labels))
            c2i = {n: i for i, n in enumerate(class_names)}
            st.markdown(f'<div class="rbox r-cyan">📂 {len(train_paths)} images &nbsp;·&nbsp; {len(class_names)} classes: {class_names}</div>', unsafe_allow_html=True)
            ...
            for ep in range(epochs):
                ...
                status.markdown(f'<div class="rbox r-cyan">Epoch {ep+1}/{epochs} &nbsp;·&nbsp; Acc: {acc_h[-1]:.4f} &nbsp;·&nbsp; Loss: {loss_h[-1]:.4f}</div>', unsafe_allow_html=True)
            mdl.save(save_path)
            st.markdown(f'<div class="rbox r-green">✅ Model saved → <code>{save_path}</code></div>', unsafe_allow_html=True)
            fig, ax = dark_fig(8, 4)
...
with t3:
    ...
    if st.button("📈  Run Evaluation"):
        if not os.path.exists(eval_path):
            st.markdown(f'<div class="rbox r-red">Model not found: <code>{eval_path}</code></div>', unsafe_allow_html=True)
        else:
            test_paths, test_raw = load_dataset_paths(TEST_DIR)
            if not test_paths:
                st.markdown(f'<div class="rbox r-red">No test data in <code>{TEST_DIR}</code></div>', unsafe_allow_html=True)
            else:
                from sklearn.metrics import classification_report, confusion_matrix, roc_curve, auc
                from sklearn.preprocessing import label_binarize
                import pandas as pd
                mdl  = load_saved_model(eval_path)
                cns  = sorted(set(test_raw)); c2i = {n:i for i,n in enumerate(cns)}
                ...
                df = pd.DataFrame(classification_report(enc, pc, target_names=cns, output_dict=True)).T
                ...
                with cb:
                    ...
                    for i, cls in enumerate(cns):
                        ...
                        ax.plot(fp, tp, color=rocs[i%4], label=f'{cls} (AUC={auc(fp,tp):.2f})', lw=1.8)
...
with t4:
    ...
    if not paths:
        st.markdown(f'<div class="rbox r-amber">No data found in <code>{data_dir}</code></div>', unsafe_allow_html=True)
    else:
        ...
        with d2:
            ...
            st.markdown(f'<div style="margin-top:.9rem;color:#6a8a9a;font-size:.8rem">Total: {total} images</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
